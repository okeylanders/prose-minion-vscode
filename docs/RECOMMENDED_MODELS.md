# Recommended AI Models for Prose Minion

Prose Minion connects to OpenRouter to provide access to the world's best AI models. Choosing the right model for the right task can significantly improve your writing experience.

Here is a guide to the models we recommend for creative writing, analysis, and editing.

---

## 🎭 Creative Prose & Storytelling

**Best for:** Writing scenes, generating dialogue, style improvement, and avoiding "AI voice"

### Claude Sonnet 4.5 — The Gold Standard

- **Best For:** Top-tier powerhouse for all creative tasks
- **Description:** Exceptional at natural prose, deep subtext, and complex narrative construction
- **Links:**
  - OpenRouter: https://openrouter.ai/anthropic/claude-sonnet-4.5
  - Anthropic: https://www.anthropic.com

### Claude 3.7 Sonnet — The All-Rounder

- **Best For:** A user favorite for balanced creative work
- **Description:** Balances exceptional lyrical prose with deep reasoning. Ideal for complex narrative rewrites
- **Links:**
  - OpenRouter: https://openrouter.ai/anthropic/claude-3.7-sonnet
  - Anthropic: https://www.anthropic.com

### Claude Opus 4.1 — Deep Storytelling

- **Best For:** The creative heavyweight for nuanced work
- **Description:** Unequalled nuance and character depth; writes with a distinctive, sophisticated voice
- **Links:**
  - OpenRouter: https://openrouter.ai/anthropic/claude-opus-4.1
  - Anthropic: https://www.anthropic.com

### TheDrummer: Anubis 70B — Visceral Prose

- **Best For:** Gritty, "human" prose that avoids AI tropes
- **Description:** An unaligned creative specialist. Excels at visceral storytelling
- **Links:**
  - OpenRouter: https://openrouter.ai/thedrummer/anubis-70b-v1.1
  - HuggingFace: https://huggingface.co/TheDrummer

### Sao10K Euryale 70B — Roleplay & Voice

- **Best For:** Capturing unique character voices
- **Description:** Creative and non-restrictive. Excellent for unconventional formatting and character work
- **Links:**
  - OpenRouter: https://openrouter.ai/sao10k/l3.3-euryale-70b
  - HuggingFace: https://huggingface.co/Sao10K

### TheDrummer: Rocinante 12B — Fluid Narrative

- **Best For:** Engaging narrative voice and varied vocabulary
- **Description:** A specialized storyteller model designed for engaging narrative flow
- **Links:**
  - OpenRouter: https://openrouter.ai/thedrummer/rocinante-12b
  - HuggingFace: https://huggingface.co/TheDrummer

### Nous Hermes 4 405B — Mature Themes

- **Best For:** Handling mature themes without moralizing
- **Description:** An uncensored frontier model. Best for complex conflicts and gritty narratives
- **Links:**
  - OpenRouter: https://openrouter.ai/nousresearch/hermes-4-405b
  - Nous Research: https://nousresearch.com/

---

## 🧠 Deep Analysis & Structural Logic

**Best for:** Dictionary definitions, Prose Analysis critiques, finding plot holes, and structural editing

### Gemini 2.5 Pro — Complex Reasoning

- **Best For:** Advanced reasoning with "thinking" capabilities
- **Description:** Strong alternative to GPT-5 for dissecting complex prose
- **Links:**
  - OpenRouter: https://openrouter.ai/google/gemini-2.5-pro
  - Google DeepMind: https://deepmind.google/technologies/gemini/

### DeepSeek R1 — Pure Logic

- **Best For:** The open-weight reasoning king
- **Description:** Excellent for heavy structural analysis, checking plot logic, and pacing verification
- **Links:**
  - OpenRouter: https://openrouter.ai/deepseek/deepseek-r1
  - DeepSeek: https://www.deepseek.com/

### xAI Grok 4 — Deep Reasoning

- **Best For:** Massive context (256k) with strong reasoning
- **Description:** Great for maintaining consistency across long narratives
- **Links:**
  - OpenRouter: https://openrouter.ai/x-ai/grok-4
  - xAI: https://x.ai/

### Mistral Magistral — Literary Analysis

- **Best For:** Deeper structural critiques
- **Description:** Mistral's dedicated reasoning model. Great for European-style prose sensibilities
- **Links:**
  - OpenRouter: https://openrouter.ai/mistralai/magistral-medium-2506
  - Mistral AI: https://mistral.ai/

### Moonshot Kimi K2 — Plot Logic

- **Best For:** Deep-dives into plot holes and character motivations
- **Description:** Reasoning-optimized for narrative consistency
- **Links:**
  - OpenRouter: https://openrouter.ai/moonshotai/kimi-k2-thinking
  - Moonshot AI: https://www.moonshot.cn/

---

## 🚀 Frontier Powerhouses

**Best for:** General tasks, heavy lifting, and versatile editing

### OpenAI GPT-5.1 — Versatility

- **Best For:** The adaptable virtuoso
- **Description:** Capable of mastering diverse tones and styles, from punchy thrillers to literary fiction
- **Links:**
  - OpenRouter: https://openrouter.ai/openai/gpt-5.1
  - OpenAI: https://openai.com/

### OpenAI GPT-5.1 Chat — Brainstorming

- **Best For:** Highly conversational and fluid
- **Description:** Excellent for dialogue workshops, brainstorming, and writing that flows naturally
- **Links:**
  - OpenRouter: https://openrouter.ai/openai/gpt-5.1-chat
  - OpenAI: https://openai.com/

### Google Gemini 3.1 Pro — Massive Context

- **Best For:** Master of scale (1M+ tokens)
- **Description:** Excellent at weaving narrative threads across massive contexts. Perfect for Series Bibles
- **Links:**
  - OpenRouter: https://openrouter.ai/google/gemini-3.1-pro-preview
  - Google DeepMind: https://deepmind.google/technologies/gemini/

---

## ⚡ Fast & Efficient

**Best for:** Category Search, quick checks, and batch processing

### Claude Haiku 4.5 — Quick Checks

- **Best For:** Anthropic's fastest efficient model
- **Description:** Perfect for quick grammar checks and style flag detection
- **Links:**
  - OpenRouter: https://openrouter.ai/anthropic/claude-haiku-4.5
  - Anthropic: https://www.anthropic.com

### Gemini 2.5 Flash — Batch Processing

- **Best For:** High-speed workhorse
- **Description:** Good for rapid critiques and checking large batches of text
- **Links:**
  - OpenRouter: https://openrouter.ai/google/gemini-2.5-flash
  - Google DeepMind: https://deepmind.google/technologies/gemini/

### xAI Grok 4 Fast — Drafting

- **Best For:** Cost-efficient regular model
- **Description:** Good for quick iterations and drafting
- **Links:**
  - OpenRouter: https://openrouter.ai/x-ai/grok-4-fast
  - xAI: https://x.ai/

---

## 💻 Specialized Coding

**Best for:** Working on the extension itself, technical writing, or complex formatting scripts

### Qwen3 Coder Plus — Coding Agent

- **Best For:** Powerhouse coding agent
- **Description:** Use for advanced formatting scripts or technical edits
- **Links:**
  - OpenRouter: https://openrouter.ai/qwen/qwen3-coder-plus
  - Qwen: https://qwenlm.github.io/

### GPT-5.1 Codex — Scripting

- **Best For:** Advanced coding specialist
- **Description:** Use for scripting or complex formatting tasks
- **Links:**
  - OpenRouter: https://openrouter.ai/openai/gpt-5.1-codex
  - OpenAI: https://openai.com/

### DeepSeek V3.1 Terminus — Tech Analysis

- **Best For:** Optimized for complex tasks
- **Description:** Strong general reasoning for structured text analysis and coding
- **Links:**
  - OpenRouter: https://openrouter.ai/deepseek/deepseek-v3.1-terminus
  - DeepSeek: https://www.deepseek.com/

---

**Note:** Model availability and pricing are subject to OpenRouter. Check the Settings menu in Prose Minion to select your preferred models.
