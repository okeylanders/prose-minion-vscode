...
|---|-----|---------|-----------|-----------|--------|
| 1 | 🟠 High | Architecture witnesses live in `packages/core` but `fs`-read `apps/vscode-extension` source — relocate to the app-side jest root reserved for them | Marcus | — | **Addressed** — moved to `apps/vscode-extension/src/__tests__/architecture/providerAssembly.test.ts`; core's suite scans core only, with a cross-reference in its header |
| 2 | 🟠 High | "Byte-identical sidebar HTML / no leak" headline claim has zero automated witness (shared vscode mock lacks `Uri.joinPath`; add mock method + snapshot both surfaces) | Cal | — | **Addressed** — mock grew `Uri.joinPath`; `webviewHtml.test.ts` pins the sidebar output by snapshot (deterministic nonce), asserts the workshop stamp, and proves the surfaces differ ONLY by title + #root |
| 3 | 🟡 Standard | `webview_error` seam: new host consumer logs unvalidated/unbounded webview text, and the event now has two divergent parsers/prefixes — the sidebar's is the broken one today | Patricia, Oliver | 🎯 | **Addressed** — ONE shared parser (`coerceWebviewErrorText` in `@shared/types`, unit-tested): validates as `unknown`, flattens newlines, caps at 500 chars; both consumers use it; UIHandler's flat-shape throw fixed (+ regression test); both surfaces log under the greppable `[WEBVIEW ERROR]` prefix, workshop tagged `(workshop)` |
| 4 | 🟡 Standard | New witnesses prove less than their comments claim: dead `(?!MessageHandler\b)` lookahead ("Handler" never in suffix list → Sprint 2's `new WorkshopHandler` uncaught); 300-char "argument window" overshoots the constructor | Cal | — | **Addressed** — suffix net now includes Handler/Adapter/Cache/Gateway/Factory (lookahead is live; `new WorkshopHandler` in a provider fails); constructor window bounded at the call's closing `);`; walker skips any `__tests__` dir so colocated fixtures can't false-trip |
| 5 | 🟡 Standard | Surface stamp `"workshop"` is two hand-synced string literals across packages — no shared symbol in `@shared/types`; ditto bare `'webview_error'` vs `MessageType.WEBVIEW_ERROR` | Marcus | — | **Addressed** — `PM_SURFACE_ATTR` / `WebviewSurface` / `SURFACE_WORKSHOP` in `@shared/types/surface.ts`, imported by both sides; bootstrap script + provider now use `MessageType.WEBVIEW_ERROR`. (workshop.css necessarily keeps the literal in attribute selectors — noted in surface.ts) |
| 6 | 🟡 Standard | `[Workshop] Panel opened (services wired: 13, platform: ok)` is a compile-time constant dressed as a runtime health check | Parker, Oliver | 🎯 | **Addressed** — now logs the truthful `[Workshop] Panel opened`, nothing more |
| 7 | 🟡 Standard | `viewType = 'proseMinion.workshop'` breaks the kebab-case id convention (`prose-minion.*`); camelCase `proseMinion.*` is the settings-key namespace | Stan, Parker | 🎯 | **Addressed** — `'prose-minion.workshop'` (safe pre-ship rename; no external references existed) |
| 8 | 🟡 Standard | Rail's "first six" tools reproduce the static comp (`pm-frames-fulltab.js`), not the approved Direction B rail override (`pm-direction-b.js` swaps in Choreography + Show & Tell) | Bria | — | **Addressed** — explicit `RAIL_TOOL_IDS` = the approved six (dialogue, prose, gestures, choreography, cliche, show-and-tell); naive slice removed; comments now cite which file supplied what |
| 9 | 🟡 Standard | Sprint doc's `Branch:` field names a never-pushed branch; the memory-bank note in the same commit has the real one | Bria | — | **Addressed** — sprint doc + epic sequencing table both name the real branch and link PR #66 |
| 10 | 🟡 Standard | `WorkshopApp` has zero `ErrorBoundary` coverage (sidebar `App.tsx` wraps every major section) | Sam | — | **Deferred** — static tree verifiably can't throw; must land with (not after) Sprint 2's dynamic content. Tracked: carried into `sprints/02-session-spine.md` tasks |
| 11 | 🟡 Standard | `WorkshopPanelProvider` behavior (reveal-if-exists, dispose lifecycle) untested — the new witnesses are DI-shape checks only | Cal | — | **Deferred** — no provider-test harness exists repo-wide; needs a small fake-panel fixture first. Tracked: noted as opportunistic in `sprints/02-session-spine.md` |
| 12 | 🟡 Standard | `retainContextWhenHidden` doubles idle webview heap; watch item: Sprint 2 must reuse the one `coreServices`, never construct a second polling service | Tim | — | **Deferred** — by ADR design for v1; add a witness when Sprint 2 wires the panel's MessageHandler. Tracked: carried into `sprints/02-session-spine.md` tasks |
| 13 | 🟢 Nit | `onDidReceiveMessage` listener not captured/disposed in the sibling's explicit `(handler, undefined, [])` shape | Stan | — | **Addressed** — subscription captured and explicitly disposed in `onDidDispose` |
| 14 | 🟢 Nit | `workshop.css` has no `:focus` rules — Sprint 2's enabled composer inherits the sidebar's unscoped focus ring and looks right only by token-inheritance coincidence | Sam | — | **Deferred** — moot while the composer is disabled; add scoped focus styles with the Sprint 2 composer work. Tracked: carried into `sprints/02-session-spine.md` tasks |
| 15 | 🟢 Nit | Nonce still `Math.random()` — pre-existing and moved verbatim, but one weak-nonce site now underwrites two surfaces | Patricia | — | **Deferred** — swap to a CSPRNG before Sprint 2 renders real content. Tracked: carried into `sprints/02-session-spine.md` tasks |
