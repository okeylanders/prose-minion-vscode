...
|---|-----|---------|-----------|-----------|--------|
| 1 | 🟠 High | Context source document embedded unbounded, bypassing the 50k-word cap its sibling evidence paths enforce; this PR triples the worst-case resend multiplier | Marcus, Cal, Bria, Blake, Tim | 🎯🎯 Strong | **Open** |
| 2 | 🟠 High | Protocol-shaped final prose (a quoted `<prose-minion-tool-call>`) is treated as a tool call → real analysis discarded / content-swapped | Sam, Patricia, Parker | 🎯 | **Open** |
| 3 | 🟠 High | Narrated-intent stream guard silently defeats progressive streaming for answers opening "Let me check…" | Sam | — | **Open** |
| 4 | 🟡 Standard | `ensureInitialized()` caches a rejected promise forever; new fire-and-forget `void` startup call has no `.catch` | Cal, Oliver, Blake | 🎯 | **Open** — cheap hardening; not reachable on today's traced paths |
| 5 | 🟡 Standard | `GuideCapability` is a shared singleton with last-write-wins mutable allowlist (vs. per-call `ContextFileCapability`); also bypasses the `AIResourceManager` factory | Patricia, Stan | 🎯 | **Open** |
| 6 | 🟡 Standard | A guide that fails to load vanishes from the model's evidence (dev-log only) | Oliver | — | **Open** |
| 7 | 🟡 Standard | Context source-read failure is downstream-indistinguishable from "no source configured" | Oliver | — | **Open** |
| 8 | 🟡 Standard | Full rejected-response log dump (may contain manuscript text) is always-on, not opt-in as the PR's own note claims | Patricia | — | **Open** |
| 9 | 🟡 Standard | WTA gained a ~50-line "Diversity & Creative Sampling" section but the sprint's required A/B comparison task is still unchecked | Bria | — | **Open** |
| 10 | 🟡 Standard | `finish_reason:'length'` truncation-notice behavior survived the refactor; its regression test did not get re-homed | Cal | — | **Open** |
| 11 | 🟡 Standard | The same 4-statement turn-bookkeeping sequence is copy-pasted 3× in `runInitial` | Parker | — | **Open** |
| 12 | 🟡 Standard | Two unrelated "is this becoming XML" mechanisms coexist in `executeTurn` with nothing linking them | Parker | — | **Open** |
| 13 | 🟡 Standard | `saxes` declared in root `package.json` instead of `packages/core/package.json` | Stan | — | **Open** |
...
| 15 | 🟢 Nit | `PromptedPassageAssistant.ts` is the only PascalCase filename in `tools/assist/` | Stan | — | **Open** |
