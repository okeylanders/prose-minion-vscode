...
|---|-----|---------|-----------|-----------|--------|
| 1 | 🟠 High | `continueConversation` cancel-race: stream finishing as abort fires appends to stored history but never shows the reply → next follow-up's context has a Q&A the user never saw | Sam | — | **Open** |
| 2 | 🟠 High | Retain block stores raw `last.content` (un-stripped `<guide-request>`) not `cleanedResponse` → MAX_TURNS-exhausted run bakes a dangling tag into follow-up context | Bria | — | **Open** |
| 3 | 🟠 High | Excerpt replacement never severs the retained conversation → follow-up continues reasoning about an excerpt no longer pinned | Marcus | — | **Open** — confirm intended scoping (does re-pin sever, or only a new tool run?) |
| 4 | 🟠 High | `handlePickExcerptFile` guard not re-checked after `readFile` → a tool run starting mid-read gets its excerpt swapped out from under it | Sam | — | **Open** |
...
| 6 | 🟡 Standard | "Pin from file…" fully reads + UTF-8-decodes the whole file into the extension host before any size check → misclicked huge file OOMs the host | Patricia | — | **Open** |
| 7 | 🟡 Standard | Out-of-workspace picks put the raw absolute path (home dir + username) in the header + log | Patricia | — | **Open** |
| 8 | 🟡 Standard | `seedExcerpt` stamps `source: 'extension.command.workshop_selection'` — a third shape matching neither `webview.workshop` nor `extension.<domain>` | Stan | — | **Open** |
| 9 | 🟡 Standard | Cancel wire is only exercised against a tool run, never a `handleSendMessage` follow-up | Cal | — | **Open** |
| 10 | 🟡 Standard | `handleCancelRequest` miss case (wrong domain / stale requestId) returns with zero log | Oliver | — | **Open** |
...
| 13 | 🟢 Nit | `wasCancelled`'s `\|\| options.signal?.aborted` clause is dead code — simplify + rename to `wasAborted` | Parker | — | **Open** — trivial cleanup |
| 14 | 🟢 Nit | `WorkshopHandler.test.ts` skipped the sibling sub-`describe` grouping convention | Stan | — | **Open** — trivial |
| 15 | 🟢 Nit | Empty-file guard logs the failure but not which file (`picked.fsPath` in scope) | Oliver | — | **Open** — trivial |
