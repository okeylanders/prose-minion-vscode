...
|---|-----|---------|-----------|-----------|--------|
| 1 | 🟠 High | A filename/label search hit silently suppresses all content search across the group — result still says `success`, and neither persona nor writer is told prose was never scanned | Sam, Bria | 🎯 | **Addressed** — catalog hits now lead a combined bounded catalog+content search; a cross-file suppression regression test pins the behavior |
| 2 | 🟠 High | CRLF files: `bytes` measured on the LF-rejoined window vs `totalBytes` on the raw file — a complete read renders as "X of Y bytes read" partial | Sam | — | **Addressed** — window and total bytes now share the normalized LF basis |
| 3 | 🟠 High | Read-truncation recount (`returnedLineCount`/`endLine`) tested only with one unbroken line — a "Lines: X–Y of Z" provenance regression passes CI | Cal | — | **Addressed** — multi-line LF and CRLF truncation fixtures pin the returned end line |
| 4 | 🟠 High | Fail-closed `catch` in the symlink-ancestor walk has zero coverage — a fail-open regression would silently admit paths to the model-visible catalog | Cal | — | **Addressed** — stat failure is forced in a resolver test and the resource remains excluded |
| 5 | 🟠 High | Byte budgets bound the *output*; nothing bounds the *input* — whole-file read/decode/split before any cap, plus a full re-encode in `sliceUtf8`; no stat-size pre-guard | Tim | — | **Addressed** — catalog stat sizes pre-bound search/read I/O, search loads incrementally, and `sliceUtf8` checks `Buffer.byteLength` before encoding |
...
| 8 | 🟡 Standard | Landing on exactly the 20-match cap sets `truncated: true` + "may not have been shown" even when nothing was omitted; `status`/`truncated` semantics differ across the three catalog/search paths | Sam | — | **Addressed** — content search now probes for a real overflow; an exhaustive exact-cap test stays successful and untruncated |
| 9 | 🟡 Standard | Four capability dispatchers share an if-chain shape but not the same implicit final case; none call `assertNever` | Parker | — | **Addressed** — all four dispatchers are exhaustive switches with `assertNever` defaults |
| 10 | 🟡 Standard | New artifact-mapping switch in `WorkshopSessionService` skips the `assertNever` default this same PR writes twice elsewhere | Stan | — | **Addressed** — artifact mapping now has an exhaustive default arm |
| 11 | 🟡 Standard | Codec comment claims parity with `ResourceReadXmlCodec`, but the sibling gates preamble by *length* (500 chars, content-blind) while the new codec gates by *content* (`<`/`>`) | Stan | — | **Addressed** — comment now documents the stricter markup-bearing-preamble rejection |
...
| 15 | 🟡 Standard | Redacted catalog-miss log carries zero attributable signal; the one caller for which it's load-bearing is the pre-existing `ContextFileCapability` path | Oliver | — | **Addressed** — miss logs now include the configured catalog size without restoring the requested path |
...
| 17 | 🟡 Standard | Immutable `base.md` claims unconditional resource access; only the per-turn instruction is catalog-aware — prompt honesty rests on the later message winning | Bria | — | **Addressed** — both autonomous-access and proactive-search wording are conditional on the turn contract advertising resources |
| 18 | 🟡 Standard | Instruction advertises two proactive research workflows (continuity + six-group bible search) inside a shared 3-call budget with no prioritization guidance | Bria | — | **Addressed** — live two-profile evidence proved a valid `search → read → search → read` chain exceeds three; the shared cap is now five (four demonstrated calls plus one spare), while one-full-entry/one-analysis subcaps remain unchanged |
| 19 | 🟢 Nit | Dead `Math.min(Number.MAX_SAFE_INTEGER, …)` guard for a case made impossible two lines earlier | Parker | — | **Addressed** — removed the dead guard |
...
Deferred findings 6, 7, 12, 13, 14, and 16 are tracked in
[Workshop persona file-access review follow-ups](../../.todo/tech-debt/2026-07-16-workshop-persona-file-access-review-follow-ups.md).
