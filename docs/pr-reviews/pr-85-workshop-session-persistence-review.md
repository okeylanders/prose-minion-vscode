...
Addressed before merge: **1–11, 13–18, and 20–22**. The two explicitly
accepted v1 risks remain deferred: **12** (whole-snapshot autosave cost) and
**19** (hostile exact-read allocation bound).

Responsibility boundaries added:

- `WorkshopSessionStateV1.ts` — durable aggregate schema and decoder entrypoint
- `WorkshopSessionStateV1Shape.ts` — exact structural grammar
- `WorkshopSessionStateV1Integrity.ts` — semantic/referential invariants
- `persistedValidation.ts` — format-neutral persisted-value primitives
- `persistedJson.ts` — unversioned JSON cloning/undefined-member policy
- `WorkshopSessionMessageHandler.ts` — named-session IPC and search cancellation
- `WorkshopSessionSearchIndexV1.ts` — compact derived browser-index codec
- `WorkshopSessionLimits.ts` — unversioned runtime aggregate limits

`V1` is deliberate only on independently persisted/decoded formats. Generic
limits, copying policy, and validation primitives remain unversioned. An
independent class-responsibility pass also reviewed every class created or
expanded by the branch. Its behavior-bearing follow-ups are recorded in
`.todo/tech-debt/2026-07-24-workshop-session-responsibility-follow-ups.md`
rather than mixed into this correction as stealth refactors.

Local verification: typecheck passed; 124 suites / 1,213 tests / 1 snapshot
passed; lint completed with 0 errors and 768 repository warnings; production
build and bundle sentinels passed; `git diff --check` passed. GitHub CI has not
yet run against these local remediation changes.
