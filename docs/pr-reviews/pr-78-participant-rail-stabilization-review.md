...
|---|-----|---------|-----------|-----------|--------|
| 1 | 🟠 High | Rail (dismiss included) unlocks at wire-complete (`phase: 'settled'`), deterministically one webview-message-tick before the assistant turn lands — "until the run completes" holds for the wire, not the transcript; the thread already computes the wider predicate (`showLiveTurn`, `WorkshopApp.tsx:289`) and the lock doesn't reuse it | Sam, Bria *(Blake dissents: same boundary `canMessage` always used; host state stays consistent)* | 🎯 | **Addressed** — the rail now shares `showLiveTurn`, so it stays locked until the settled bubble is replaced by the completed turn |
| 2 | 🟠 High | Locked chips still advertise clickable copy — `title` still says "Talk to X" / "Back to Jill" and the dismiss `aria-label` is silent about the lock, while the component's own `unavailable` branch demonstrates self-explaining disabled copy two lines away | Marcus, Oliver, Quinn | 🎯🎯 | **Addressed** — all four locked tooltip branches and the dismiss label now explain that the control is available after the response finishes |
| 3 | 🟠 High | "disables every control" test renders 3 of 5 control kinds — no guest chip, no dismiss button (the one control that got new CSS this PR and the one with host-side stakes); `disabled` and a live guest never co-occur anywhere in the suite | Marcus, Sam, Cal | 🎯🎯 | **Addressed** — the locked fixture now renders host, live guest, dismiss, tool, and invite controls and asserts all five disabled |
| 4 | 🟡 Standard | App-level wiring untested: the unit test hand-passes `disabled`/`showInviteGuest`, no `WorkshopApp` test exists (searched — none), and the PR's own named regression shape (Jill-only room: empty sidecars/guests, invite-only rail) is never rendered. *(One sub-claim normalized in compilation: the feared mid-run null window from `sessionReady` settling doesn't exist — the host snapshot delivers `sessionReady` + `activeRequestId` in one message and React batches the updates.)* | Cal, Bria | 🎯 | **Partially addressed** — the empty/empty invite-only locked case is covered; a root `WorkshopApp` render harness is tracked in [tech debt](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md) |
| 5 | 🟡 Standard | Focus falls to `<body>` when a focused rail control flips to native `disabled`; no focus management added (searched component/App/test — none). *(Compilation note: the click-chip-then-send path moves focus off the rail before the lock lands; the realistic narrow path is modal return-focus onto the now-disabled invite chip after starting a guest-join run.)* | Sam, Quinn | 🎯 | **Addressed** — the toolbar accepts programmatic focus and takes focus when a contained control becomes disabled; regression covered |
| 6 | 🟡 Standard | `role="toolbar"` with every child natively disabled is unreachable to keyboard/AT users mid-run — "visible but locked" is delivered to sighted mouse users only; the `aria-disabled` + click-guard alternative would introduce a pattern with no precedent in this codebase | Quinn | — | **Deferred** — deliberate pattern decision tracked in [tech debt](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md); native `disabled` remains the house pattern |
| 7 | 🟡 Standard | Stuck-lock has no self-heal: every clearing signal is a fire-and-forget `postMessage` (no ack, no visibility-change resync, one-shot session request on mount), and in that same scenario Cancel silently no-ops on requestId mismatch (host logs "Cancel ignored", sends nothing back). Pre-existing delivery-layer gap; this PR changes its face — chips now sit dimmed-forever instead of absent | Oliver | — | **Deferred** *(infrastructure, untouched by this diff)* — visibility/reconnect resync and authoritative stale-cancel recovery are tracked in [tech debt](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md) |
| 8 | 🟡 Standard | `WORKSHOP_SET_CHAT_TARGET` is the one mutating rail action never revalidated against an active run — invite and dismiss both consult `activeRun`/`preemptActiveRun()`; `handleSetChatTarget` and `WorkshopSessionService.setChatTarget` check target validity only. DOM path is genuinely blocked by this PR; message-channel path isn't. Pre-existing, bounded consequence (routing-focus display race) | Patricia | — | **Deferred** — host-boundary revalidation and its preempt-or-reject policy are tracked in [tech debt](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md) |
| 9 | 🟡 Standard | Dismiss-to-cancel granularity lost: pre-PR, dismissing the streaming guest was a working, host-supported cancel path (`handleDismissGuest` still preempts that exact run — the branch is now UI-unreachable during runs). Blunt composer Cancel remains. Deliberate simplification or unexamined loss — the PR body doesn't say | Bria | — | **Addressed** — intentional blanket lock retained; the PR body now names composer Cancel as the supported in-flight cancellation path |
| 10 | 🟡 Standard | `disabled` prop's doc comment ("Keep the participant map visible…") narrates the **caller's** visibility fix, not the prop's mechanism — the prop has no hand in visibility; the null-guard + the removed `!isRunning` term in `WorkshopApp` do that work. Future maintainer chasing "rail vanished" reads it and looks in the wrong file | Parker | — | **Addressed** — the prop comment now describes only control disabling; the component header narrates the stable locked rail behavior |
| 11 | 🟡 Standard | Invite-visibility rule is an unnamed 4-line inline boolean in JSX; the same file already demonstrates the fix (`showLiveTurn`: hoisted, named, one-line-commented, reused) | Parker | — | **Addressed** — hoisted and documented as `canInviteGuest` |
| 12 | 🟡 Standard | Load-bearing `22px` lands uncommented while the sibling rules in this exact ticker→rail→budget band all explain their numbers; the two halves of the gap contract live in two files (`index.css` / `workshop.css`) with no cross-reference | Parker | — | **Addressed** — the Workshop-scoped budget margin now documents the stable ticker-to-budget contract |
| 13 | 🟡 Standard | Prop named `disabled` (modal family: multi-cause, folded by caller) though the wiring is a bare single-cause `workshop.isRunning` passthrough — exactly the shape `ExcerptPanel` in the same band names `isRunning` *(Parker dissents: the name matches the modals and mechanically that's all it does — genuine panel split)* | Stan | — | **Addressed** — the caller now passes the wider `showLiveTurn` predicate, so the mechanism-level `disabled` name is intentionally retained |
| 14 | 🟡 Standard | `renderRail` helper not extended — second test now bypasses it with inline JSX; `WorkshopPersonaBrowserModal.test.tsx:11` shows the destructured-options-param pattern that solves this | Stan | — | **Deferred** — options-object cleanup is tracked in [tech debt](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md) for the next fixture-matrix expansion |
| 15 | 🟢 Nit | Component header comment still narrates only pre-lock chip semantics; `ExcerptPanel`'s header got a sentence when comparable interaction behavior landed (commit 6868cf2) | Stan | — | **Addressed** — header now documents mounted-but-locked behavior during active responses |
...
The remaining app-harness work from row 4 and deferred rows 6, 7, 8, and 14 are tracked in [Workshop participant-rail review follow-ups](../../.todo/tech-debt/2026-07-17-workshop-participant-rail-review-follow-ups.md). Row 13 resolved in favor of `disabled` once the caller began folding the wider transcript-aware predicate.

### Remediation validation — 2026-07-17

- `npm test -- --runInBand` — 97 suites, 834 tests, 1 snapshot passed
- `npm run typecheck` — core, webview, and extension projects passed
- `npm run lint` — 0 errors; 701 existing warnings
- `npm run build` — production extension and webview bundles passed verification
- `git diff --check` — passed

The blanket lock is intentional: guest dismissal remains unavailable during an
active response, and the composer Cancel action is the supported in-flight
cancellation path. This keeps every participant-routing mutation under the same
temporary lock instead of preserving a special dismiss-to-cancel exception.

---

## Blast Radius (reviewed patch)
