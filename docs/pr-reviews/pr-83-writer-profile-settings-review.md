...
This is careful, trust-conscious work: the privacy boundary is **structural** rather than conventional, the injection frame is neutralized on both fields, prototype pollution is foreclosed, and the tests that *do* exist are honest (one of them greps the session snapshot to prove the bio never lands in it). But two confirmed blockers share a single root — `VsCodeSettingsStore` validates the profile's *shape* while never asking about its *scope or provenance* — so a repo can inject prompt content (Patricia) and a failed write can silently revert a cleared bio back into live prompts (Blake). **Not merge-ready as-is:** the two blockers, the fail-closed test blind spot that hid them, and the ADR-vs-code divergence on the deleted rows should be resolved first. The encouraging part is that the fixes are localized — a `scope` declaration, an echo-guard on the reconcile, and the tests that push a real profile through the seam — and the foundation underneath is genuinely sound.
