...
**Status:** 🟢 **Merge-ready — review fixups completed in `d6d7316`.** Blake cleared every correctness path. The panel's `StandardsService.getGenres()` cluster was re-traced and corrected: `PublishingStandardsRepository` construction performs no I/O and cannot produce the alleged missing-repository state; real file/JSON failures still reject through `getGenres()` and surface a webview ERROR. The fixup makes the repository field required and adds a regression test for that actual failure path. The second-webview lifecycle test, directory-wide architecture guard, typed publishing transport mock, and stale-comment cleanup also landed. The remaining items are explicitly deferred watch-items or pre-existing nits.
...
|---|----------|---------|-------------|-------------|
| 1 | ⚪ Corrected | **`StandardsService.getGenres()` returns `[]` on repo-load failure where the old per-request path surfaced a webview ERROR.** Old `PublishingHandler` did `new PublishingStandardsRepository(...).getGenres()` inside its try/catch → a construction failure threw → `sendError('publishing', …)` reached the webview. New path: `loadStandards()` swallows the construction error (logs at cold-boot, leaves `standardsRepo` undefined) → `getGenres()` hits `if (!this.standardsRepo) return []` → handler posts a **success** payload with zero genres. Silent empty panel instead of an error. | Bria · Blake · Marcus · Sam · Oliver · Cal **🎯🎯 Strong Consensus** | **Resolved by correction + hardening (`d6d7316`).** The repository constructor only stores dependencies; it performs no I/O and does not create the alleged failure state. The optional field and defensive `[]` branch were therefore dishonest. The field is now required/readonly, and a regression test proves real `readFile`/JSON failures reject through `getGenres()` and become a publishing ERROR. |
| 2 | 🟡 Standard | **Shared services carry per-webview mutable status-emitters.** `CategorySearchService`, `AssistantToolService`, `DictionaryService` are built once at the composition root but their `statusEmitter` is set per `MessageHandler` construction and cleared on dispose. Ownership is split: the root owns the object, `MessageHandler` owns a critical callback. Works today because dispose→rebuild is atomic in `resolveWebviewView`; nothing type-level stops a stale emitter. | Marcus | **Deferred intentionally.** Correct at current scope and now covered across dispose → second construction. Passing emitters per call would be a broader service-contract redesign, not a merge fixup. |
| 3 | 🟡 Standard | **Triple-write to `resultCache`.** `sendStatus`/`sendError`/`applyTokenUsage` write `this.resultCache.*` directly, then call `this.postMessage(...)`, whose spy-switch writes the **same** keys again. Two answers to "who owns cache population." | Parker | **Deferred intentionally.** Status/token writes are redundant, but `sendError()` also performs broader cache clearing than the central switch does for `extension.handler` errors. Centralization needs explicit behavior tests and should not be a mechanical review cleanup. |
| 4 | 🟡 Standard | **Dispose test covers a single handler only.** The PR's headline lifecycle risk is the *second* webview reusing the shared `accountBalanceService`; the suite asserts `dispose()` fires once but never builds a second handler on the same assembly to prove re-subscription works. | Cal | **Resolved (`d6d7316`).** The assembly test now disposes the first handler, constructs a second against the same shared services, and verifies balance-listener, status-emitter, and token-callback re-subscription. |
| 5 | 🟡 Standard | **`boundaries.test.ts` infra-construction guard reads only `MessageHandler.ts`.** The `\bnew\s+(TextSourceResolver\|…)\b` regex is correct but file-scoped; a *domain handler* that `new`s infrastructure passes silently. The sibling vscode-import guard correctly scans the whole tree via `collectSourceFiles`. | Cal | **Resolved (`d6d7316`).** The guard now scans every TypeScript file under `application/handlers/` and reports offending relative paths. |
| 6 | 🟡 Standard | **Callback-detach `catch {}` in `dispose()` silently skips the rest if any setter throws.** The five `setXxx(undefined)` calls share one try-block; an early throw leaves later services with stale bound closures. | Oliver | **Deferred — non-blocking.** The setters are currently simple assignments/iteration and the next construction self-heals. Improve alongside a broader teardown-observability pass. |
| 7 | 🟡 Standard (Medium) | **`platform.secrets` and `SecretStorageService` both derive from `context.secrets` via two paths; no test pins them to the same store.** Theoretical: a future maintainer swapping `platform.secrets` for a double wouldn't realize `SecretStorageService` was built from it. | Patricia | **Deferred — no active defect.** Both paths intentionally share `context.secrets`; revisit if the platform secret adapter gains alternate implementations. |
| 8 | 🟢 Nit | **`PublishingHandler.test.ts` still casts `postMessage` as `any`** — 19 lines into the first test of the very `MessageTransport` seam this PR introduced. | Stan | **Resolved (`d6d7316`).** The mock now uses `MessageTransport`; the same suite also covers the publishing ERROR contract. |
| 9 | 🟢 Nit | **Stale `SPRINT 05` comments survive** in `MessageHandler.ts` (file header + method tags) and now mislabel the bundle-injection path as "direct service injection." | Parker | **Resolved (`d6d7316`).** Historical sprint labels were removed or replaced with current lifecycle intent. |
| 10 | 🟢 Nit | **`as unknown as CoreServices` in the assembly test** defeats the compiler — adding a required field to `CoreServices` won't fail the test, blunting the guard the suite is meant to be. | Parker · Cal | **Deferred.** Concrete service classes make a fully structural test double noisy; replace this with dedicated port-shaped contracts when the service bundle is narrowed further. |
| 11 | 🟢 Nit | **Balance-dispose `catch {}` guards operations that can't throw** (`Set.delete`, `clearTimeout` + `Set.clear`). Silence doing theater. | Oliver | **No change — informational only.** Suitable for a future teardown cleanup, not a merge gate. |
...
Implemented in `d6d7316`:

- Made `StandardsService.standardsRepo` required and readonly, removing an impossible optional state.
- Added a publishing-handler regression test proving repository read failures still surface as webview ERROR messages.
- Extended the lifecycle assembly test through dispose and construction of a second handler using the same shared services.
- Widened the infrastructure-construction guard to all files under `application/handlers/`.
- Replaced the `PublishingHandler` transport test cast from `any` to `MessageTransport`.
- Removed stale `SPRINT 05` labels from `MessageHandler`.

Intentionally deferred:

- Mutable per-webview emitters on shared services remain a documented watch-item; current teardown/rebuild behavior is tested.
- Result-cache write centralization needs dedicated error-cache behavior tests because `sendError()` and `postMessage()` do not currently clear identical domains.
- Optional teardown/logging, secret-store commentary, concrete `CoreServices` test-double typing, and pre-existing nonce work remain non-blocking follow-ups.

Validation after fixups:

- 49/49 Jest suites passing
- 374/374 tests passing
- Core, webview, and extension TypeScript checks passing
- ESLint: zero errors (existing warning backlog remains)
- Production extension/webview build and bundle sentinel verification passing
...
⚪ **Post-review correction:** the strong-consensus `getGenres()` finding assumed `PublishingStandardsRepository` construction could fail and leave the service without a repository. The constructor performs no I/O; actual file/JSON failures occur later in `getGenres()` and already reach `PublishingHandler`'s ERROR path. Fixup `d6d7316` removed the impossible optional state and added a regression test for the real failure path.
...
### Lesson 2 — Trace the Actual Failure Point

Illuminated by: the `getGenres` consensus (Bria · Blake · Marcus · Sam · Oliver · Cal)

The panel agreed strongly on a failure-path regression that did not exist because it assigned I/O behavior to a constructor that only stores dependencies. The useful instinct was correct—refactors must preserve failure contracts—but the trace stopped before identifying where failure can actually occur. Here, file reading and JSON parsing remain lazy inside `getGenres()`, where rejection still reaches the handler's visible ERROR path.

→ Carry forward: for every moved call, identify the exact throwing operation before comparing old and new error paths. Then pin that real failure with a regression test.
...
A disciplined, well-tested, well-documented architectural consolidation that lands exactly what ADR 2026-06-18 promised: `extension.ts` is the single composition root again, the boundaries are typed (`MessageTransport`/`SecretsPort`/`CoreServices`), the replay cache is instance-bound, and an architecture witness guards the invariant going forward. **No blockers, no highs.** Fixup `d6d7316` closes the actionable coverage and guard gaps, makes the standards-repository invariant explicit, and proves the real publishing failure path still surfaces an ERROR. The mutable-emitter ownership seam remains a documented architectural watch-item, not a merge blocker.
