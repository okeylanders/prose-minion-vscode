...
import { WORKSHOP_SYNTHETIC_MEDIA_FIXTURES } from '@/__tests__/fixtures/workshopMultimodalFixtures';
...
describe('Workshop composer attachment policy', () => {
  ...
  it('keeps one tiny synthetic signature fixture for every accepted format', () => {
    const accepted = Object.entries(WORKSHOP_COMPOSER_ATTACHMENT_POLICY.media)
      .flatMap(([mediaKind, policy]) => Object.keys(policy.formats)
        .map((format) => `${mediaKind}:${format}`))
      .sort();
    const fixtures = WORKSHOP_SYNTHETIC_MEDIA_FIXTURES
      .map(({ mediaKind, format }) => `${mediaKind}:${format}`)
      .sort();

    expect(fixtures).toEqual(accepted);
    expect(new Set(fixtures).size).toBe(fixtures.length);
    for (const fixture of WORKSHOP_SYNTHETIC_MEDIA_FIXTURES) {
  ...
  it('maps every fixture extension and MIME type through the accepted policy', () => {
    for (const fixture of WORKSHOP_SYNTHETIC_MEDIA_FIXTURES) {
      const policy = WORKSHOP_COMPOSER_ATTACHMENT_POLICY.media[fixture.mediaKind];
