...
export const WORKSHOP_WIDGET_RECOMMENDATION_INSTRUCTION = [
  ...
  'This is a quality-first handoff, not a token-saving exercise. Do not be thrifty, terse, or generically minimal in the prefill fields. Supply enough grounded material that the dictionary model can understand the dramatic problem without reconstructing it from scraps:',
  ...
  '- `surrounding-context`: copy a generous, consecutive stretch of the supplied prose around the phrase—normally the full relevant beat and useful paragraphs before and after, up to the field limit. Preserve the source wording; never summarize, pad, or invent passage text.',
  ...
  'All four fields are required when you recommend the widget. If the supplied material cannot support them honestly, omit the recommendation. Everything remains editable and nothing runs until the writer chooses it. Do not explain widget mechanics in prose—the chip and prefilled form do that.',
...
export function inspectWorkshopWidgetRecommendation(
  ...
  if (
    ...
    || !onlyBlankBetween(SURROUNDING_CONTEXT_END, CHARACTER_NOTES_START)
