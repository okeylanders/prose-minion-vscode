...
const RESERVED_PERSONA_FRAME =
  /<\/?(?:pinned-excerpt|context-attachments?|writer-message|workshop-tool-evidence|workshop-host-update|workshop-todo-snapshot|writer-owned-task|workshop-capability-result|workshop-transcript|workshop-guest-catch-up|workshop-guest-handoff|workshop-excerpt-source|workshop-open-conversation|workshop-analysis-scope|workshop-interaction-transition|workshop-behavior-activation|workshop-interaction|workshop-writer-profile|workshop-session-attunement|workshop-time-context|thread-artifact|agent-artifact|prose-minion-tool-call)(?=[\s/]|>)[^>]*>/gi;
