...
Use `<excerptText>` only with excerpt `prepend` or `replace`, and `<contextText>` only with context `prepend` or `replace`. `prepend` is the only way to add run-specific framing or instructions above existing material; there is no separate instructions field. Persona-supplied excerpt text may contain at most 10,000 words and 120,000 characters. Persona-supplied context text may contain at most 50,000 words and 420,000 characters. These ceilings apply to persona-supplied text, not inherited material that the host has already bounded. `omit` plus `omit` is legal. Never put a filesystem path in either text field.
