...
| ~~**React 17 → 18**~~ | RESOLVED in Pass 2 Wave 1: React/ReactDOM 18, webview `createRoot`, and hook tests migrated to `@testing-library/react` to avoid the React 17 renderer pulled by `@testing-library/react-hooks`. | ✅ Pass 2 Wave 1 |
...
| Item | Blocker |
|---|---|
| ~~**Pass 2 — Design Facelift artifacts**~~ | RESOLVED: the Claude Design URL was actually attempted first and hit a Cloudflare/403 auth wall; the author supplied `.temp/Prose Minion-handoff.zip`, which contains the sidebar HTML/CSS/JS mock and assets. FrameMinion remains the implementation reference. |
